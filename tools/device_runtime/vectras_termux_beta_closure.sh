...
# It surveys every gate before returning a fail-closed status.
set -u
umask 077
...
Usage: vectras_termux_beta_closure.sh [--audit|--repair|--enterprise]
  ...
  --repair      staged pkg repair + full inspection; RAFCODEPHI gate required
...
mkdir -p "$BASE_OUT" 2>/dev/null || { echo "cannot create receipt directory: $BASE_OUT" >&2; exit 3; }
...
PKG_OUT="$BASE_OUT/.pkg-path.$$"
PKG_ERR="$BASE_OUT/.pkg-err.$$"
if /system/bin/cmd package path "$PKG_APP" >"$PKG_OUT" 2>"$PKG_ERR"; then
  APP_PATH_LINE="$(head -n 1 "$PKG_OUT")"
  ...
  if [ -r "$APP_APK" ] && command -v sha256sum >/dev/null 2>&1; then
    ...
    record ANDROID_PACKAGE TOKEN_VAZIO apk_sha256 "APK path not externally readable; in-app evidence hashes sourceDir"
  fi
else
  record ANDROID_PACKAGE BLOCKER installed "$(head -n 1 "$PKG_ERR" 2>/dev/null || echo package_not_visible)"
fi
rm -f "$PKG_OUT" "$PKG_ERR" 2>/dev/null || true
...
check_app_path files/evidence/vectras-runtime-latest.json APP_RUNTIME_RECEIPT exists
...
LATEST_TMP="$BASE_OUT/.latest.$$"
cp "$RECEIPT" "$LATEST_TMP" 2>/dev/null && mv "$LATEST_TMP" "$LATEST" || true
printf '\nReceipt: %s\n' "$RECEIPT"
printf 'Summary: PASS=%d FAIL/BLOCKER=%d BLOCKER=%d TOKEN_VAZIO=%d\n' "$passes" "$failures" "$blockers" "$token_vazios"

# Full survey first, fail-closed status second. Any real FAIL/BLOCKER fails closure.
