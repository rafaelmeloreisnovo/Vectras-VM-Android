...
def test_transient_reset_retries_then_succeeds() -> None:
    ...
    rc = module.run_with_retry(
        ["./tools/ci/build_apk_wizard.sh"], attempts=4,
        base_backoff_seconds=1, runner=runner, sleeper=sleeps.append,
    )
    assert rc == 0
    assert len(calls) == 3
    assert sleeps == [1, 2]
...
def test_http_503_is_retryable() -> None:
    ...
    rc = module.run_with_retry(
        ["same", "pinned", "command"], attempts=3,
        base_backoff_seconds=0, runner=runner, sleeper=lambda _delay: None,
    )
    assert rc == 0
    assert len(calls) == 2
...
def test_http_404_fails_without_retry() -> None:
    ...
    rc = module.run_with_retry(
        ["same", "pinned", "command"], attempts=4,
...
def test_transient_exhaustion_is_bounded() -> None:
    ...
    rc = module.run_with_retry(
        ["same", "pinned", "command"], attempts=3,
        base_backoff_seconds=2, runner=runner, sleeper=sleeps.append,
    )
    assert rc == 7
    assert len(calls) == 3
    assert sleeps == [2, 4]
