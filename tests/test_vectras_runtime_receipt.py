...
def receipt(receipt_id: str, created: int, *, claim: bool = True, apk: str = "b" * 64) -> dict:
    ...
    return {
        ...
        "claim_allowed": claim,
        "reason": "private-runtime-hash-and-exec-probes-pass" if claim else "runtime-gap",
    }
...
class VectrasRuntimeReceiptTest(unittest.TestCase):
    ...
    def test_two_distinct_identically_bound_runs_reproduce(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            a = validator.validate(self.write(root, "a.json", receipt("vectras-run-0006", 1005)))
            b = validator.validate(self.write(root, "b.json", receipt("vectras-run-0007", 1006)))
    ...
    def test_reproduction_rejects_apk_change(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            a = validator.validate(self.write(root, "a.json", receipt("vectras-run-0008", 1007, apk="1" * 64)))
            b = validator.validate(self.write(root, "b.json", receipt("vectras-run-0009", 1008, apk="2" * 64)))
            reproduced = validator.reproduce(a, b)
    ...
    def test_reproduction_rejects_device_fingerprint_change(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            a_payload = receipt("vectras-run-0010", 1009)
            b_payload = receipt("vectras-run-0011", 1010)
