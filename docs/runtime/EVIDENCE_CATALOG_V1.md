...
Filesystem identity in an exported receipt is limited to these logical roots and
approved runtime/evidence subpaths:
...
<filesDir>/evidence/catalog/...
<installedApk>/base.apk
<nativeLibDir>/...
```

Any external, unknown, sibling, guest, unapproved `filesDir` child, or
canonicalization-failure path is emitted as:
