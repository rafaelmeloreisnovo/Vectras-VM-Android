...
void RmR_LL_ApplyTuneDefaults(const RmR_HW_Info *hw, RmR_LL_TunePlan *plan) {
...

  {
    ...
    plan->qemu_smp_cpus = rmr_clamp_u32(plan->qemu_smp_cpus, 2u, 16u);

    plan->policy_batch_size = rmr_clamp_u32(line * 128u, 2048u, 65536u);
    if (l2_kib >= 1024u) plan->policy_batch_size = rmr_clamp_u32(plan->policy_batch_size * 2u, 4096u, 131072u);

    plan->policy_lane_width = native_64 ? 16u : 8u;
    if (l4_mib >= 16u) plan->policy_lane_width = 32u;
    plan->policy_lane_width = rmr_clamp_u32(plan->policy_lane_width, 4u, 32u);

    plan->policy_commit_quantum = native_64 ? 32u : 16u;
    if (l4_mib >= 16u) plan->policy_commit_quantum = 64u;

    plan->cti_chunk_size = rmr_clamp_u32(line * 64u, 1024u, 65536u);
    plan->cti_stride = (hw->has_cycle_counter || hw->has_asm_probe) ? 2u : 1u;
    plan->cti_prefetch = rmr_clamp_u32(line * 4u, 64u, 1024u);
  }
}
...
void RmR_VcpuScheduler_Init(RmR_VcpuScheduler *sched, uint32_t vcpu_count) {
  ...
  sched->vcpu_count = rmr_clamp_u32(vcpu_count, 1u, 16u);
